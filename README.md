...
- `PatternWindowMex("displayDynamicMemory", index_list, delay=0, verbose=true, use_parallel=true)` - display the dynamic memory at the specified index (0-start) and delay is the wait time in milliseconds after displaying each pattern. Function will return a boolean value indicating if the display timing is expected for the current refresh rate in case of a dropped frame
- `success = PatternWindowMex("displayDynamicMemoryAll", delay=0, verbose=true, use_parallel=true)` - display all the dynamic memory and delay is the wait time in milliseconds after displaying each pattern. Function will return a boolean value indicating if the display timing is expected for the current refresh rate in case of a dropped frame
...
- `PatternWindowMex("generateBlackTweezerPattern", coord, radius, shift_per_move, num_RGB_buffer, num_moving_frames, use_parallel=true)` - generate a black tweezer pattern with the specified parameters in the internal dynamic memory, coord is a Nx2 double array of coordinates in pixel, radius (double) is the radius of the tweezer in pixel, shift_per_move (double) is the 1x2 distance to move the tweezer in pixel, num_RGB_buffer is the number of RGB buffer to use, num_moving_frames is the number of frames to move the tweezer, delay is the delay time in milliseconds between each frame, use_parallel is a boolean value indicating if parallel processing should be used
