...
### SDL3 (version 3.1.8 pre-release)
...
1. Download the SDL3 source code from the SDL3 repository. The example here is for `SDL-preview-3.1.8` version. Newer versions of SDL3 may be available in the future with bug fixes and new features.
...
```bash
cmake -S ./SDL-preview-3.1.8 -B ./SDL3_build -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release -DSDL_SHARED=OFF -DSDL_STATIC=ON
```
...
7. Copy all the header files from the source directory include folder `SDL-preview-3.1.8\include\SDL3` to the `DMD-SDL/include` directory.
